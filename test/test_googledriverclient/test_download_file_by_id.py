...
class TestDownloadFileById(unittest.TestCase):
    ...
    def test_with_default_name(self):
        file = self.client.get_file_by_path(
            'test/test-1/test-1.1/test-1.1.1.txt')

        self.assertIsNotNone(file)

        local_file_path = self.client.download_file_by_id(
            file['id'], local_folder_path='./')
    ...
    def test_with_custom_name(self):
        ...
        local_file_path = self.client.download_file_by_id(
            file['id'], local_folder_path='./', local_file_name=custom_name)
